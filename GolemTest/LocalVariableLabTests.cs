...
public sealed class LocalVariableLabTests
{
    ...
    private const string NextOrderLocal = @"
        {
    ...
    public void MayTheRouteUnderway_LiveInALocalVariable_InAQuery_AnEmit_AndACommand()
    {
        ...
        try
        {
            perf.Actor.Reactions.DefineReaction("next-order-visit").Cue().Company().WithSharedHydration()
                .Seek("Act").One().OnMatch("[_:Golem].Visit(_, _)").Program.Emit(NextOrderLocal);
        ...
        try
        {
            string f = perf.Actor.Using("{ me = Pose(@x, @y, @t); g.Wake(me); print g.Standing.X 'x', g.Standing.Y 'y'; }")
                .WithParameters(p => { p["x", typeof(double)] = 6.3; p["y", typeof(double)] = 10.4; p["t", typeof(double)] = 0.0; }).PerformCommand();
        ...
        perf.Actor.Using("{ from = Pose(@fx, @fy, @ft); point = Position(@px, @py); route = g.Visit(from, point); }")
            .WithParameters(p => { p["fx", typeof(double)] = 6.3; p["fy", typeof(double)] = 10.4; p["ft", typeof(double)] = 0.0; p["px", typeof(double)] = 2.0; p["py", typeof(double)] = 9.5; }).PerformCommand();
        Thread.Sleep(1500);
        ...
        try { findings.Add("A query braced local -> " + perf.Actor.Using(NextOrderLocal).PerformQuery().Replace("\n", " ")); }
        ...
        try { findings.Add("B query unbraced local -> " + perf.Actor.Using("if (g.HasPendingMission()) { route = g.Underway(); print route.Id 'route', route.Order 'action'; }").PerformQuery().Replace("\n", " ")); }
        ...
        try
        {
            string d = perf.Actor.Using("{ me = Pose(@px, @py, @ptheta); g.Wake(me); }\n" + NextOrderLocal)
                .WithParameters(p => { p["px", typeof(double)] = 5.5; p["py", typeof(double)] = 10.0; p["ptheta", typeof(double)] = 3.1; }).PerformCommand();
        ...
        try { findings.Add("E global 'route' after D -> LEAKED: " + perf.Actor.Using("print route.Id 'v';").PerformQuery()); }
