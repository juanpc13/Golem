...
public sealed class ReactionPatternLabTests
{
    ...
    public void WhichPatterns_MatchTheBracedActs_WithoutAnExpose()
    {
        ...
        perf.Actor.Using(@"
            {
                from = Position(@fx, @fy);
                point1 = Position(@x, @y);
                route = g.Visit(from, point1);
                point2 = Position(@x2, @y2);
                route.Then(point2);
            }
        ")
            .WithParameters(p => {
                ...
                p["x2", typeof(double)] = 9.0;
                p["y2", typeof(double)] = 1.5;
            }).PerformCommand();
        perf.Actor.Using(@"
            {
                route = g.Find(@id);
        ...
        ")
            .WithParameters(p => {
                p["id", typeof(int)] = 1;
                p["vx", typeof(double)] = 2.0;
                p["vy", typeof(double)] = 1.5;
            }).PerformCommand();
        perf.Actor.Using(@"
            {
                route = g.Find(@id);
        ...
        ")   // resumed: the turn is asked again
            .WithParameters(p => {
                p["id", typeof(int)] = 1;
                p["x", typeof(double)] = 2.0;
                p["y", typeof(double)] = 9.5;
                p["t", typeof(double)] = -1.745;
            }).PerformCommand();   // resumed from the kitchen: the door's approach lies south-south-west
        perf.Actor.Using(@"
            {
                route = g.Find(@id);
                me = Pose(@x, @y, @t);
        ...
        ")
            .WithParameters(p => {
                p["id", typeof(int)] = 1;
                p["x", typeof(double)] = 3.0;
                p["y", typeof(double)] = 9.5;
                p["h", typeof(double)] = 3.14;
                p["px", typeof(double)] = 3.25;
