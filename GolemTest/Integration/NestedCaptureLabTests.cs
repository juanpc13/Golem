...
public sealed class NestedCaptureLabTests
{
    ...
    public void CanAReaction_CaptureTheValuesInsideANestedConstructor_WithoutAnExpose()
    {
        var cases = new (string Name, string Pattern, string Emit)[]
        {
            ...
            ("N1-bump-inline-ctor",   "[_:Golem].Bump(Pose($bx, $by, $bh), $bearing)", @"
                print @bx 'bx', @by 'by', @bh 'bh', @bearing 'bearing';
            "),
            ("N2-bump-bearing-only",  "[_:Golem].Bump(_, $bearing)",                    @"
                print @bearing 'bearing';
            "),
            // the bump as the host writes it TODAY — the pose in a local variable `me` on the line before
            ("N3-bump-var-capture",   "[_:Golem].Bump($me, $bearing)",                  @"
                print @bearing 'bearing';
            "),
            ("N4-bump-var-ctor",      "[_:Golem].Bump(Pose($bx, $by, $bh), $bearing)", @"
                print @bx 'bx', @bearing 'bearing';
            "),
            // a bare constructor pattern beside the act: does the pattern list see the `me = Pose(…)` statement?
            ("N5-ctor-then-bump",     "Pose($bx, $by, $bh) [_:Golem].Bump(_, $bearing)", @"
                print @bx 'bx', @by 'by', @bh 'bh', @bearing 'bearing';
            "),
            ("N6-ctor-alone",         "Pose($bx, $by, $bh)",                             @"
                print @bx 'bx', @by 'by', @bh 'bh';
            "),
            // the forget, inline and two-line
            ("N7-forget-inline",      "[_:Golem].Forget(Position($x, $y))",              @"
                print @x 'x', @y 'y';
            "),
            ("N8-forget-var",         "[_:Golem].Forget(Position($x, $y))",              @"
                print @x 'x', @y 'y';
            "),
            // the arrival, inline: the pose the body reports
            ("N9-arrive-inline",      "[_:Route].Arrive(Pose($x, $y, $h))",              @"
                print @x 'x', @y 'y', @h 'h';
            "),
            // MIXED: the constructor beside the act PLUS a reduced expose — what the speech would need (ajuste 42)
            ("N10-bump-mixed",        "Pose($bx, $by, $bh) [_:Golem].Bump(_, $bearing) expose $who who;", @"
                print @bx 'bx', @by 'by', @bh 'bh', @bearing 'bearing', @who 'who';
            "),
            ("N11-arrive-mixed",      "Pose($x, $y, _) [_:Route].Arrive(_) expose $rid rid, true reached;", @"
                print @x 'x', @y 'y', @rid 'rid';
            "),
            ("N12-arrive-mixed-noStop", "Pose($x, $y, _) [_:Route].Arrive(_) expose $rid rid, true reached;", @"
                print @x 'x', @y 'y', @rid 'rid';
            "),
            ("N13-forget-ctor-beside", "Position($x, $y) [_:Golem].Forget(_)",           @"
                print @x 'x', @y 'y';
            "),
        };
        ...
        foreach (var (name, pattern, emit) in cases)
        {
            ...
            {
                "N1-bump-inline-ctor" or "N2-bump-bearing-only" => @"
                    {
                        route = g.Bump(Pose(@bodyX, @bodyY, @bodyHeading), @bearing);
                    }
                ",
                "N3-bump-var-capture" or "N4-bump-var-ctor" or "N5-ctor-then-bump" or "N6-ctor-alone" => @"
                    {
                ...
                "N7-forget-inline" => @"
                    {
                        g.Forget(Position(@bodyX, @bodyY));
                    }
                ",
                "N8-forget-var" or "N13-forget-ctor-beside" => @"
                    {
                        at = Position(@bodyX, @bodyY);
                        g.Forget(at);
                    }
                ",
                "N10-bump-mixed" => @"
                ...
                "N11-arrive-mixed" => @"
                    ...
                    expose @rid rid, @stop reached;
                ",
                "N12-arrive-mixed-noStop" => @"
                    {
                        route = g.Underway();
                        me = Pose(@bodyX, @bodyY, @bodyHeading);
                        route.Arrive(me);
                    }
                    expose @rid rid, @nostop reached;
                ",
                _ => @"
                    {
                        route = g.Underway();
                        route.Arrive(Pose(@bodyX, @bodyY, @bodyHeading));
                    }
                ",
            };
