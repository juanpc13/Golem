...
**Panel = proyecciones, no lectura del journal**: view reactions `.Cue()` por hecho (`Assigned`, `Taken`, `Completed`, `Failed`, `Retired`, `Acked`) cuyo `Emit` imprime el comando TAL COMO quedó en el journal (`print 'g.Assign(' + @mission + ', ' + @x + ', ' + @y + ');' 'command', …`) más los campos que el mapa necesita, empujadas a `PanelSink : IOutputSink` (registrado con `perf.OutputTarget`) → SSE. Cada fila muestra `#entry  g.Assign(7, 9.0, 8.0);` con el nombre del hecho como etiqueta; la entrada 1 (la cadena de releases, script literal que ninguna reaction ve) la anuncia el host. Se fue el decodificador binario del journal (`JournalPeek`/`StageHook.OnRecordWritten`).
