...
**Fallo**: turtlesim no falla solo, así que hoy lo define el loop del host: atasco (la distancia deja de mejorar 3 s mientras empuja) o timeout (90 s) → `MissionFailed` con su razón → `g.Fail(@id, @reason)` journaleado. El golem NO conoce su mundo todavía (ver *Pendientes de diseño*).
