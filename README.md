...
└───────┬────────┘                                                    └───────┬────────┘
        ...
        │  /golem/<body>/result ↑ done|bumped|stuck   order ↓ ticket + action + amount   │
        └────────────────────────────┬────────────────────────────────────────┘
                                     │
                          ┌──────────┴───────────┐
...
(not versioned) with `{"Lab": {"World": "gazebo"}}`. A golem of the fleet that does not answer is left out, and a scenario that needs it is inconclusive. The deployed fleet is let go, its obstacles forgotten and the crates
...
| `route = g.Follow(point)` | The golem follows its leader to a point a peer says it reached — planned at once from where the golem knows its body stands (`g.Standing`, or where its way underway ends); refused before any act brought the pose (18-sep-2026). |
| `g.Wake(me)` | The golem wakes where its body stands — the first act of every boot (18-sep-2026): it keeps the pose (`g.Standing`, what every act brings it) so a told point is planned the moment it arrives, and a plan underway is decided again from there inside. |
| `route.Decide(from)` · `route.DecidePast(who, me)` | Repertoire: the way decided again from a point — the route does it by itself when it must (a stranded retreat, the waking), the host writes no `decide`; out of a peer's way the first leg is the courtesy step the route chooses. |
...
| `/golem/<body>/order` (to the body) `{"order": 17, "action": "advance", "amount": 1.35, "speed": 2.0}`; `/golem/<body>/result` (from it) `{"order": 17, "result": "done"}`, `"bumped"` (x, y, heading, bearing), `"stuck"` (reason) | Where the BODY reports (`sim/bridge/body.py`) — its whole vocabulary: it arrived (a turn made, a point reached), it bumped (the motors stopped at once; what to do is the golem's), it is stuck. The endpoint writes the act and the print of that act is the next order, sent back to the body |
