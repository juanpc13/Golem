...
└───────┬────────┘                                                    └───────┬────────┘
        ...
        │  HTTP /robot/arrived|bump|stuck ↑ reports (turn/run/back ↓)         │
        └────────────────────────────┬────────────────────────────────────────┘
                                     │
                          ┌──────────┴───────────┐
...
| `GolemAPI/` | The generic golem program (ASP.NET). One image, N golems by environment. `Membrane/` (rosbridge, the tell wire), `Choreography/` (`Robot` — the actor's output target: every print parsed, switched on and sent to the body over the websocket; `GolemSpeech` — the tell reactions and uptakes), `Panel/` (the page and the journal tap), `Controllers/` (`GolemController`: EVERY journal script, each ending in the print of the next order, which the command returns to whoever performed it; the robot reports on `/robot/arrived`, `/robot/bump`, `/robot/stuck`). |
