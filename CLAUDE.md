...
- **The domain is tested as OBJECTS, in C#** ("el testing es sobre el dominio de las clases, no sobre los scripts"): a
  ...
  `Order`… directly, and asserts on what the classes answer. one file per class under test — `BodyTests`, `MapLayoutTests`, `CollisionsTests`, `RoutePlannerTests`, `RouteTests`, `GolemTests` — over `DomainFixture` (the three modules built as the releases build them, the walk as the robot reports it), all under `GolemTest/Domain/` (Juan, 23-sep-2026: two folders, so a glance tells the domain's tests from the end-to-end ones). `GolemTest/EndToEnd/` holds what runs the actor with the assembly: `JournalAcceptanceTests` keeps the few checks only the engine can give, and the labs (`*LabTests`) record what the engine does. The host's own unit tests (`CapabilitiesTests`, `RequestValidationTests`) stay at the project's root. A script
