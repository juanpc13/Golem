...
- **A command template is a braced block, step by step, values as `@params`** (Juan, 10-sep): the object is
  ...
  **The body does ONE thing at a time; the journal says what, and every script lives in the GolemEmbodiment's action methods**
  ...
  back, turnLeft, turnRight, stop, continue; the bump is what it reports.** **The body gets its own words alone, and says only what came of them** (Juan, 24-sep-2026, ajuste 55: "el robot como tal debe abstraerse tanto que no sepa casi nada de quien lo comanda… sólo decir si logré lo que me dijiste que hiciera, más nada"): over the websocket (rosbridge, `std_msgs/String` on `/golem/<body>/order`) travels `{"order": 17, "action": "advance", "amount": 2.35, "speed": 2.0}` — a TICKET the mechanics stamped (`Order.Ticket`, a counter), the action, the amount and the cruise of the body declared; nothing of the route, the leg or the point (those stay the golem's: the mechanics tells the feed the whole order as a kind `order` event, for the log, the panel and the lab). Back comes `/golem/<body>/result`: `{"order": 17, "result": "done"}`, `{"order": 17, "result": "bumped", "x", "y", "heading", "bearing"}`, `{"order": 17, "result": "stuck", "reason"}` — the body's only word, on its own topic, to whoever listens: the wire (`IBodyWire.ResultReported`, `Rosbridge` subscribed) hands it to `GolemEmbodiment.Resulted`, which gives it to the role (`Displacer.Arrived(order)`, `CollisionCaptor.Bumped(…)`, `Displacer.Stuck(order, reason)`); the ticket tells a stale word from the order carried, order by order. The `/robot/*` endpoints are gone: the body is no HTTP client and knows no golem's URL. The same order twice is not resent; a different one
  ...
  and says what came of it on `/golem/<body>/result` (ajuste 55); `stop` keeps what was left of the amount, `continue` finishes it. **The body's vocabulary is its base actions** (Juan, 16-sep: "la interfaz del
  ...
  — each with its `amount` (metres, radians) and the ticket — `stop`, `continue` — and it says, on `/golem/<body>/result`, `done` (the amount
  done — the host knows which act it was from the order carried, by the ticket, and brings the body's pose to it from telemetry) or `bumped` (`{x, y, heading,
  bearing}`: where it stood facing which way, and where on its shell it was pressed — the touch on the plane is the domain's
  to reckon) — or `stuck` with why, when it could not (ajuste 55; the `/robot/*` endpoints are gone). **The bumper is a switch**: the moment it fires the
  ...
  One bump per contact: while the bumper stays pressed against the same thing it is not a new bump. Each result is
  parsed by the embodiment (`Resulted`) and the role writes the act whole (`route.Arrive(me)`, the pose from telemetry; `g.Bump(me, @bearing)`): its print is the next order — "gira, luego ros le dice ya giré, se escribe que giró y esa escritura
  ...
  relays the body's words, nothing more; the TICKET the body echoes (`order`) is the host's token for a stale word alone (ajuste 55: was the route id). When nothing comes through a print (a told point taken up as a `Follow`, a boot) the GolemEmbodiment's
...
- `sim/` — the world: Gazebo Fortress in kiosk mode. **Two published ports, no more**:
  ...
  (advance, back, turnLeft, turnRight, stop, continue — each with its amount and a ticket), drives `/model/<body>/cmd_vel`, and says what came of it on
  `/golem/<body>/result` (done, bumped, stuck — ajuste 55, 24-sep-2026: no HTTP, no golem's URL) — the robot is only the body; the golem tells it what to do, one thing at a time. Reality is
...
- **The domain is tested as OBJECTS, in C#** ("el testing es sobre el dominio de las clases, no sobre los scripts"): a
  ...
  `Order`… directly, and asserts on what the classes answer. one file per class under test — `BodyTests`, `MapLayoutTests`, `CollisionsTests`, `RoutePlannerTests`, `RouteTests`, `GolemTests` — — no fixture: EVERY TEST INSTANTIATES ITS OWN OBJECTS in its first lines (Juan, 24-sep-2026: "que los testcase dentro instancien como tal los objetos para ver completo todo") — `var map = Catalog.Warehouse(); var collisions = new Collisions(); var body = new Body(new Meters(0.25), new MetersPerSecond(2.0), new Seconds(6.0), new Meters(0.6)); var g = new Golem(body, map, collisions);`, as `init` builds them — and EVERY TEST GOES THROUGH THE GOLEM (Juan, 24-sep-2026: "que todos usen el golem, en el sentido que le pasen los módulos que se están testeando"): the module under test is handed to a golem and exercised by the golem's verbs, the way the journal writes them — a mark is born of `g.Bump(new Pose(10.25, 6.1, -1.5708), 0.0)` (where the body stood, where on its shell) or of a peer's word, `g.HearBump("red", …)`, never of `collisions.Mark`; a way of `g.Visit(from, stop).AsPlan()`, never of `planner.Road`; a hold of `g.Pause(me)`; an annulment of the peer's word landing on my body, never of `Unmark` + `Unbump` — and read where the journal reads: the golem where the body enters the answer (`g.FitsAt`, `g.Distance`, `g.RouteLength`), the module where the module answers alone (`collisions.MarkCount`, `collisions.All()`, `map.Find`, `map.Connects`, `map.ZoneNameOf`). `BodyTests` shows what each magnitude DOES (the radius in where the body fits and a follower's standoff, the retreat in the `back` after a touch, the speed and the linger in `RouteSeconds`); `MapLayoutTests` shows what the map disposes in the WAYS decided over it (a straight run, a bend on a pivot, a door crossed, no way between islands), keeping the release's text and the map's own reads; `RoutePlannerTests` is the planner reached through `g.Visit`/`g.Cover`. Every value is written where it is used: `new Position(…)`, `new Pose(…)` with the heading as a number, a refusal as `StringAssert.Contains(Assert.ThrowsException<GolemDomainException>(…).Message, …)`; the one helper a class keeps is the body's walk (`Step`, `Reach`, `WalkToTheEnd`: the turn or the advance the route asks, reported as the robot would), private to the class that walks, all under `GolemTest/EndToEnd/` (Juan, 23-sep-2026: two folders, so a glance tells the domain's tests from the integration ones; renamed 24-sep-2026: `Domain` → `EndToEnd` — "los flujos enteros de los procedimientos que esperaría que corran bajo el capó del API", the whole flows the API runs, through the golem — and the old `EndToEnd` → `Integration`, the actor with the assembly and the worlds). `GolemTest/Integration/` holds what runs the actor with the assembly: `JournalAcceptanceTests` keeps the few checks only the engine can give, and the labs (`*LabTests`) record what the engine does. **The scenarios' asserts are what the EMULATOR tells** (Juan, 24-sep-2026: "lo que me interesa es lo que el simulador me puede decir… una vez hecho el send, los asserts son para cuando un tramo llegó a su destino; cada arrive es algo que el emulador nos está contando"): after `world.Send(…)` a scenario asserts ONE LEG AT A TIME, as the body walked it — `var door = await LegWalkedAsync(world, "red"); Assert.AreEqual("living/south", door.Leg); Assert.IsTrue(door.Reached); AssertStands(door, 4.6, 1.5, …)` — one assert per leg of the way, with the pose the body REPORTED, never the golem's plan (`AsPlan()` is the domain's tests' business). `World/BodyReports` meets the two words on the TICKET (ajuste 55): the golem's — every order it told its body, whole, as a kind `order` event on its feed (the mock hears its in-process `PanelFeed`; Gazebo the `/events` stream) — and the body's — the result on `/golem/<body>/result` (the mock's `KinematicBody` says it; Gazebo over rosbridge), with the world's true pose at that moment; a leg is the run of orders toward one named leg (a door: approach then exit) and is done when every order has its result and the next order heads elsewhere, or the advance to the errand's last stop is done; a bump or a stuck cuts it. Nothing is read from the journal's acts. Pattern set on `AFreeWay` (24-sep-2026); the other scenarios follow it next. The host's own unit tests (`CapabilitiesTests`, `RequestValidationTests`) stay at the project's root. A script
