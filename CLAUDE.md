...
- **The domain is tested as OBJECTS, in C#** ("el testing es sobre el dominio de las clases, no sobre los scripts"): a
  ...
  `Order`… directly, and asserts on what the classes answer. one file per class under test — `BodyTests`, `MapLayoutTests`, `CollisionsTests`, `RoutePlannerTests`, `RouteTests`, `GolemTests` — over `DomainFixture` (the three modules built as the releases build them, the walk as the robot reports it). `JournalAcceptanceTests` keeps the few checks only the engine can give. A script
